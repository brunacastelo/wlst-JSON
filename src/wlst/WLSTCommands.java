...
import wlst.objects.beans.Bridge;
import wlst.objects.beans.BridgeTarget;
...
import wlst.objects.beans.DefaultDistributedQueue;
import wlst.objects.beans.DefaultQueue;
...
import wlst.objects.beans.JmsModuleItem;
...
public class WLSTCommands {
	...
	private Map<ITEM, List<String>> generated = new HashMap<WLSTCommands.ITEM, List<String>>();
	
	private final String QUEBRA_DE_LINHA = "\r\n";
	private final String IDENTACAO = "\t";
	private Map<String, List<Target>> clusters = new HashMap<String, List<Target>>();
	private List<ITEM> ignoredItens;
	
	StringBuilder jythonGerado;
	
	private void validateGenerated(ITEM tipo, String name) {
		List<String> values = generated.get(tipo); 
		if (values != null && values.contains(name)) {
			throw new RuntimeException("[" + tipo.toString() + ":" + name + "] ja existe " );
		}
		if (values == null) {
			values = new ArrayList<String>();
			this.generated.put(tipo,  values);
		}
		values.add(name);
	}
	...
	public WLSTCommands(ITEM ...itens) {
		interpreter = new WLSTInterpreterWrapper();
		this.ignoredItens = Arrays.asList(itens);
	}
	...
	private boolean canGenerate(ITEM item) {
		return !ignoredItens.contains(item);
	}
...
//	}
	...
	public String createConfiguration(Enviroment env) throws Exception {
		try {
			...
			if (env.getDomain() != null){
				if (canGenerate(ITEM.DOMAIN)) {
					this.createDomain(env.getDomain());
				}
			}
						
			if (env.getDataSources() != null) {
				if (canGenerate(ITEM.DATA_SOURCE)) {
					for (DataSource data : env.getDataSources()) {
						this.createDatasource(data);
					}
				}
			...
			if (env.getFileStores() != null) {
				if (canGenerate(ITEM.FILE_STORE)) {
					for (FileStore file : env.getFileStores()) {
						this.createFileStore(file);
					}
				}
			...
			if (env.getJmsServer() != null) {
				if (canGenerate(ITEM.JMS_SERVER)) {
					for (JmsServer server : env.getJmsServer()) {
						this.createJmsServer(server);
					}
				}
			...
			if (env.getJmsModule() != null) {
				if (canGenerate(ITEM.JMS_MODULE)) {
					for (JmsModule module : env.getJmsModule()) {
						this.createJmsModule(module);
					}
				}
			...
			if (env.getWorkManager() != null) {
				if (canGenerate(ITEM.WORK_MANAGER)) {
					for (WorkManager workManager : env.getWorkManager()) {
						this.createWorkManager(workManager);
					}
				}
			...
			if (env.getBridgeTargets() != null) {
				if (canGenerate(ITEM.BRIDGE)) {
					for (BridgeTarget target : env.getBridgeTargets()) {
						this.createBridgeTarget(target);
					}
				}
			}

			if (env.getBridges() != null) {
				if (canGenerate(ITEM.BRIDGE)) {
					for (Bridge bridge : env.getBridges()) {
						if (bridge.getBridgeTargets() != null) {
							for (BridgeTarget target : bridge.getBridgeTargets()) {
								this.createBridgeTarget(target);
							}
						}
	...
	private void generateCommonFunctions() {
		...
		//FUNCTION CRIAR FILA				
		this.jythonGerado.append("def CriarFila(type, moduleName, name, jndiName, subdeployment, priorityDestinationKey):");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("print ('Criando Fila: ' + name )");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cd('/')");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			...
			this.jythonGerado.append("if (type == 'QUEUE'):");
				...
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append("myq=create(name,'Queue')");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("else:");
				...
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append("myq=create(name,'UniformDistributedQueue')");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);			
		...
		this.jythonGerado.append("def CriarDeliveryfailure(moduleName, type, queueName, destinationKey, redeliveryDelay, redeliveryLimit):");
			...
			this.jythonGerado.append("else:");
				...
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append("print ('cmo.setErrorDestination(getMBean(''/JMSSystemResources/'+moduleName+'/JMSResource/'+moduleName+'/UniformDistributedQueues/'+destinationKey+'))')");
				this.jythonGerado.append(QUEBRA_DE_LINHA);
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append(IDENTACAO);
		...
		this.jythonGerado.append("def CriarCluster(name, multicastAddress, port, address):");	
			...
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("if (multicastAddress != 'null'):");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append(IDENTACAO);
				...
				this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("if (port != 0):");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append(IDENTACAO);
			...
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("if (address != 'null'):");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append(IDENTACAO);
		...
		this.jythonGerado.append("def CriarManagedServer(name, port, address, machine, cluster):");	
			...
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("if (cluster != 'null'):");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cmo.setMachine (getMBean('/Machines/' + machine))");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			...
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cmo.setCluster (getMBean('/Clusters/' + cluster))");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(QUEBRA_DE_LINHA);
		...
		this.jythonGerado.append("def CriarConnectionPool(datasourceName, driverClassName, url, dbPassword, dbUsername, inCapacity, maxCapacity, incrementCapacity, isTestConnReserve, testTableName, shrinkFrequency, connCreationRetryFrequency, incativeConnectionTimeoutSeconds, connReserveTimeout, maxCapacityInit, minCapacityInit, statementCache, testFrequency, supportsGlobal):");		
			...
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("set('MaxCapacity',maxCapacity)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
		...
		//FUNCTION CRIAR CriarBridgeTarget		
		this.jythonGerado.append("def CriarBridgeTarget(name, adapterJNDIName, connectionFactoryJNDIName, connectionURL, userName, userPassword, destinationJNDIName):");	
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("print ('Criando CriarBridgeTarget: ' + name )");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cd('/')");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cd('JMSBridgeDestinations')");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("create(name)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cd(name)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("set('AdapterJNDIName', adapterJNDIName)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("set('ConnectionFactoryJNDIName', connectionFactoryJNDIName)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("set('ConnectionURL', connectionURL)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("set('DestinationJNDIName', destinationJNDIName)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("if (userName != 'null'):");
				this.jythonGerado.append(QUEBRA_DE_LINHA);
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append(IDENTACAO);			
				this.jythonGerado.append("set('UserName', userName)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("if (userPassword != 'null'):");
				this.jythonGerado.append(QUEBRA_DE_LINHA);
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append(IDENTACAO);
				this.jythonGerado.append("set('UserPassword', userPassword)");
				this.jythonGerado.append(QUEBRA_DE_LINHA);
				this.jythonGerado.append(QUEBRA_DE_LINHA);
			
		//FUNCTION CRIAR CriarBridge		
		this.jythonGerado.append("def CriarBridge(name, qualityOfService, started, selector, targetOri, targetDest, preserveMsgProperty):");	
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("print ('Criando CriarBridge: ' + name )");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cd('/')");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cd('MessagingBridges')");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("create(name)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			...
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("set('QualityOfService', qualityOfService)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("set('Started', started)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("if (selector != 'null'):");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("set('Selector', selector)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("set('PreserveMsgProperty', preserveMsgProperty)");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cmo.setSourceDestination(getMBean('/JMSBridgeDestinations/' + targetOri))");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(IDENTACAO);
			this.jythonGerado.append("cmo.setTargetDestination(getMBean('/JMSBridgeDestinations/' + targetDest))");
			this.jythonGerado.append(QUEBRA_DE_LINHA);
			this.jythonGerado.append(QUEBRA_DE_LINHA);
}
	...
	private void createJdbcStore(JdbcStore jdbc) {
		this.validateGenerated(ITEM.JDBC_STORE, jdbc.getName());
	...
	private void createDomain(Domain domain) {
		this.validateGenerated(ITEM.DOMAIN, domain.getName());
		if (domain.getAdminServer() != null) {
			this.createAdminServer(domain.getAdminServer());
		}

		if (domain.getMachines() != null) {
			if (canGenerate(ITEM.MACHINE)) {
		...
		if (domain.getClusters() != null) {
			this.clusters = new HashMap<String, List<Target>>();
			if (canGenerate(ITEM.MACHINE)) {
		...
		if (domain.getManagedServers() != null) {
			for (ManagedServer managedServer : domain.getManagedServers()) {
				if (canGenerate(ITEM.MANAGED_SERVER)) {
					this.createManagedServer(managedServer);
				}
				if (managedServer.getCluster() != null) {
					List<Target> servers = clusters.get(managedServer.getCluster());
					if (servers == null) {
						servers = new ArrayList<Target>();
						clusters.put(managedServer.getCluster(), servers);
					}
					servers.add(new Target(managedServer.getName(), Target.TargetType.Server));
				}
			}
		}
	...
	private void createMachines(Machine machine) {
		this.validateGenerated(ITEM.MACHINE, machine.getName());
		this.cd("/");
	...
	private void createManagedServer(ManagedServer managedServer) {
		this.validateGenerated(ITEM.MANAGED_SERVER, managedServer.getName());
	...
	private void createDatasource(DataSource data) {
		this.validateGenerated(ITEM.DATA_SOURCE, data.getJndiName());
	...
	private void createFileStore(FileStore file) {
		this.validateGenerated(ITEM.FILE_STORE, file.getName());
	...
	private void createJmsServer(JmsServer jmsServer) {
		this.validateGenerated(ITEM.JMS_SERVER, jmsServer.getName());
	...
	private void createAuxJmsModuleItem(String moduleName, JmsModuleItem item) {
		if (item.getSubdeployment() == null && item.getTargets() == null) {
			throw new RuntimeException("Topico deve ter subdeployment ou target");
		} else if(item.getSubdeployment() == null && item.getTargets() != null) {
			if (item.getSubdeployment() == null && item.getTargets() != null) {
				List<JmsServer> servers = new ArrayList<JmsServer>();
				String baseName = item.getName().startsWith("sis.") ? item.getName().substring(4) : item.getName();
				if (baseName.endsWith("Queue")) {
					baseName = baseName.substring(0, baseName.lastIndexOf("Queue"));
				}
				List<Target> subDeploymentTargets = new ArrayList<Target>();
				for (Target target : item.getTargets()) {
					List<Target> clusterTargets;
					if (target.getType().equals(Target.TargetType.Cluster)) {
						clusterTargets = this.clusters.get(target.getName());
					} else {
						clusterTargets = Arrays.asList(target);
					}
					for (Target clusterTarget : clusterTargets) {
						// FileStore
						String fileStoreName = "Filestore_" + baseName + "_" + clusterTarget.getName();
						FileStore fileStore = new FileStore(fileStoreName, "./FileStore/" + fileStoreName, clusterTarget.getName(), Target.TargetType.Server.name());
						this.createFileStore(fileStore);
						// JmsServer
						String jmsServerName = "JMSServer_" + baseName + "_" + clusterTarget.getName();
						JmsServer jmsServers = new JmsServer(jmsServerName, fileStore.getName(), clusterTarget.getName(), Target.TargetType.Server.name(), 0, 0); 
						this.createJmsServer(jmsServers);
						servers.add(jmsServers);
						Target t = new Target(jmsServerName, Target.TargetType.JMSServer);
						subDeploymentTargets.add(t);
					}
				}
				// SubDeployment
				Subdeployment subdeployment = new Subdeployment("SubDeployment_" + baseName, subDeploymentTargets, servers);
				this.createSubdeployment(moduleName, subdeployment);
				item.setSubdeployment(subdeployment.getName());
			} else if(item.getSubdeployment() == null && item.getTargets() == null) {
				throw new RuntimeException("Topico deve ter subdeployment ou target");
			}
		}
	...
	private void createJmsModule(JmsModule jmsModule) {
		this.validateGenerated(ITEM.JMS_MODULE, jmsModule.getName());
		...
		if (jmsModule.getTopics() != null) {
			for (Topic topic : jmsModule.getTopics()) {
				this.createAuxJmsModuleItem(jmsModule.getName(), topic);
				this.createTopics(jmsModule.getName(), topic);
			}
		}
		
		if (jmsModule.getDefaultDistributedQueues() != null) {
			for (DefaultDistributedQueue defaultQueue : jmsModule.getDefaultDistributedQueues()) {
				for (String jndiName : defaultQueue.getQueues()) {
					DistributedQueue queue = new DistributedQueue(jndiName,
							                                      defaultQueue.getSubdeployment(),
							                                      defaultQueue.getTemplate(),
							                                      defaultQueue.getDeliveryFailure(),
							                                      defaultQueue.getDestinationKeys(),
							                                      defaultQueue.getPriorityDestinationKey(),
							                                      defaultQueue.getTargets(),
							                                      jndiName);
					this.createAuxJmsModuleItem(jmsModule.getName(), queue);
					this.createDistributedQueues(jmsModule.getName(), queue);
				}
			}
		}
		
		if (jmsModule.getDefaultQueues() != null) {
			for (DefaultQueue defaultQueue : jmsModule.getDefaultQueues()) {
				for (String jndiName : defaultQueue.getQueues()) {
					Queue queue = new Queue(jndiName,
							                defaultQueue.getSubdeployment(),
							                defaultQueue.getTemplate(),
							                defaultQueue.getDestinationKeys(),
							                defaultQueue.getDeliveryFailure(),
							                defaultQueue.getPriorityDestinationKey(),
							                defaultQueue.getTargets(), jndiName);
					this.createAuxJmsModuleItem(jmsModule.getName(), queue);
		...
		if (jmsModule.getDistributedQueues() != null) {
			for (DistributedQueue distQueue : jmsModule.getDistributedQueues()) {
				this.createAuxJmsModuleItem(jmsModule.getName(), distQueue);
	...
	private void createQueue(String type, String moduleName, Queue queue) {
		this.validateGenerated(ITEM.QUEUE, queue.getName());
		...
		if (queue.getDeliveryFailure() != null && "Redirect".equals(queue.getDeliveryFailure().getExpirationPolixy())) {
			this.validateGenerated(ITEM.QUEUE, this.getErrorDestinationName(queue));
			this.jythonGerado.append("CriarFila('");		
			this.jythonGerado.append(type);
			this.jythonGerado.append("','");		
			this.jythonGerado.append(moduleName);
			this.jythonGerado.append("','");		
			this.jythonGerado.append(this.getErrorDestinationName(queue));
			this.jythonGerado.append("','");		
			this.jythonGerado.append(this.getErrorDestinationName(queue));
			this.jythonGerado.append("','");		
		...
		this.jythonGerado.append(moduleName);
		this.jythonGerado.append("','");		
		this.jythonGerado.append(queue.getName());
		this.jythonGerado.append("','");		
		...
		if (queue.getDeliveryFailure() != null && "Redirect".equals(queue.getDeliveryFailure().getExpirationPolixy())) {
			this.createDeliveryFailure(moduleName, type, queue.getName(), this.getErrorDestinationName(queue), queue.getDeliveryFailure());
		}

	}
	private String getErrorDestinationName(Queue queue) {
		String errorDestination = queue.getDeliveryFailure().getErrorDestination();
		if (errorDestination == null || "".contentEquals(errorDestination.trim())) {
			errorDestination = queue.getName();
			if (errorDestination.endsWith("Queue")) {
				errorDestination = errorDestination.substring(0, errorDestination.lastIndexOf("Queue"));
			}
			errorDestination += "DLQ";
		}
	...
	private void createQueueNetwork(String type, String moduleName, Queue queue) {
		...
		this.jythonGerado.append(moduleName);
		this.jythonGerado.append("','");		
		this.jythonGerado.append(queue.getName()+"_DLQ");
		this.jythonGerado.append("','");		
		this.jythonGerado.append(queue.getName()+"_DLQ");
		this.jythonGerado.append("','");		
		this.jythonGerado.append(queue.getSubdeployment());
		...
		if (queue.getDeliveryFailure() != null && "Redirect".equals(queue.getDeliveryFailure().getExpirationPolixy())) {
			...
			this.createDeliveryFailure(moduleName, type, queue.getJndiName(), errorDestination, queue.getDeliveryFailure());
		}
	}
	...
	private void createQueueProv(String type, String moduleName, Queue queue) {
		...
		this.jythonGerado.append(moduleName);
		this.jythonGerado.append("','");		
		this.jythonGerado.append(queue.getName()+"DLQ");
		this.jythonGerado.append("','");		
		this.jythonGerado.append(queue.getName()+"DLQ");
		this.jythonGerado.append("','");		
		this.jythonGerado.append(queue.getSubdeployment());
		...
		if (queue.getDeliveryFailure() != null && queue.getDeliveryFailure().getRedeliveryDelayOverride() != 0) {
			this.createDeliveryFailure(moduleName, type, queue.getName(), queue.getDestinationKeys(), queue.getDeliveryFailure());
		}
	...
	private void createSubdeployment(String moduleName, Subdeployment subdeployment) {
		this.validateGenerated(ITEM.SUBDPLOYMENT, subdeployment.getName());
	...
	private void createTopics(String moduleName, Topic topic) {
		this.validateGenerated(ITEM.TOPIC, topic.getName());
		this.jythonGerado.append("CriarTopic('");		
		this.jythonGerado.append(moduleName);
		this.jythonGerado.append("','");		
		this.jythonGerado.append(topic.getName());
		this.jythonGerado.append("','");		
		this.jythonGerado.append(topic.getSubdeployment());		
		this.jythonGerado.append("',");		
	...
	private void createConnectionFactory(String moduleName, ConnectionFactory connectionFactory) {
		this.validateGenerated(ITEM.CONNECTION_FACTORY, connectionFactory.getName());
	...
	private void createBridgeTarget(BridgeTarget target) {
		this.validateGenerated(ITEM.BRIDGE_TARGET, target.getName());
		// this.jythonGerado.append("def CriarBridgeTarget(name, adapterJNDIName, connectionFactoryJNDIName, connectionURL, userName, userPassword, destinationJNDIName):");
		this.jythonGerado.append("CriarBridgeTarget('");
		this.jythonGerado.append(target.getName());
		this.jythonGerado.append("','");
		this.jythonGerado.append(target.getAdapterJNDIName());
		this.jythonGerado.append("','");
		this.jythonGerado.append(target.getConnectionFactoryJNDIName());
		this.jythonGerado.append("','");
		this.jythonGerado.append(target.getConnectionURL());
		this.jythonGerado.append("','");
		this.jythonGerado.append(target.getUserName());
		this.jythonGerado.append("','");
		this.jythonGerado.append(target.getUserPassword());
		this.jythonGerado.append("','");
		this.jythonGerado.append(target.getDestinationJNDIName());
		this.jythonGerado.append("')");
		this.jythonGerado.append(QUEBRA_DE_LINHA);
	}
	
	private void createBridge(Bridge bridge) {
		this.validateGenerated(ITEM.BRIDGE, bridge.getName());
		// this.jythonGerado.append("def CriarBridge(name, qualityOfService, started, targetOri, targetDest, preserveMsgProperty):");
		this.jythonGerado.append("CriarBridge('");
		this.jythonGerado.append(bridge.getName());
		this.jythonGerado.append("','");
		this.jythonGerado.append(bridge.getQualityOfService());
		this.jythonGerado.append("','");
		this.jythonGerado.append(bridge.getStarted());
		this.jythonGerado.append("','");
		this.jythonGerado.append(bridge.getSelector());
		this.jythonGerado.append("','");
		this.jythonGerado.append(bridge.getTargetOri());
		this.jythonGerado.append("','");
		this.jythonGerado.append(bridge.getTargetDest());
		this.jythonGerado.append("','");
		this.jythonGerado.append(bridge.getPreserveMsgProperty());
		this.jythonGerado.append("')");
		this.jythonGerado.append(QUEBRA_DE_LINHA);
		this.setTargets(bridge.getTargets());
	}	
